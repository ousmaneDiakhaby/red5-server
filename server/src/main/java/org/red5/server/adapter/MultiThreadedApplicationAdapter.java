...
public class MultiThreadedApplicationAdapter extends StatefulScopeWrappingAdapter implements ISharedObjectService, IBroadcastStreamService, IOnDemandStreamService, ISubscriberStreamService, ISchedulingService, IStreamSecurityService, ISharedObjectSecurityService, IStreamAwareScopeHandler, IWebSocketAwareHandler, ApplicationMXBean {
    ...
    public void measureBandwidth(IConnection conn) {
        // dummy for now, this makes flv player work they dont wait for connected status they wait for onBWDone
    ...
    private IProviderService getProviderService() {
        if (providerService == null) {
            try {
                providerService = (IProviderService) ScopeUtils.getScopeService(scope, IProviderService.class, Class.forName("org.red5.server.stream.ProviderService"));
            } catch (ClassNotFoundException e) {
                log.warn("Default provider service class not found", e);
                providerService = (IProviderService) ScopeUtils.getScopeService(scope, IProviderService.class);
            }
        }
